import DesignCard from "./DesignCard"
const DesignThought = () => {
    return <div className="text-white">
        <h1 className="text-6xl md:text-7xl lg:text-9xl font-bold">3 YEARS OF</h1>
        <h1 className="text-6xl md:text-7xl lg:text-9xl font-bold text-gray-600">EXPERIENCE</h1>
        <DesignCard
            title="PixelForge Studios"
            description="Led the design team in creating user-centric mobile and web applications, improving the user experience and increasing user engagement."
            startDate="Jan 2020"
            endDate="Present"
        />
         <DesignCard
            title="PixelForge Studios"
            description="Led the design team in creating user-centric mobile and web applications, improving the user experience and increasing user engagement."
            startDate="Jan 2020"
            endDate="Present"
        />
         <DesignCard
            title="PixelForge Studios"
            description="Led the design team in creating user-centric mobile and web applications, improving the user experience and increasing user engagement."
            startDate="Jan 2020"
            endDate="Present"
        />
    </div>

}
export default DesignThought